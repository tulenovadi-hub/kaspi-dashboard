...
const RED_KEYS = new Set(['cost_of_goods', 'returns', 'cost_of_returns', 'commission', 'delivery', 'taxes', 'marketing', 'ff_services', 'other_expenses']);
...
const MAIN_COLUMNS = [
  ...
  { key: 'ff_services', label: 'Услуги ФФ' },
  ...
  return (
    <div>
      ...
      <div className="report-note">
        ...
        «Маркетинг» — сумма расходов на рекламу («Маркетинг», данные из Kaspi Pay) за тот же месяц по всем кампаниям сразу; эти расходы не привязаны
        к городу отгрузки, поэтому считаются по всему магазину целиком, а не только по Алматы и Астане. «Услуги ФФ» — расходы на фулфилмент-центр
        (упаковка/обработка заказов и хранение на складе), загружаются PDF-отчётами на странице «Расходы»; тоже считаются по всему магазину целиком.
        ...
        выручке. «Маркетинг» на уровне товара считается через привязку рекламной кампании к товару (если кампания продвигает сразу несколько товаров —
        её расход делится между ними поровну; кампании без сохранённой привязки к товару в разбивку не попадают). «Прочие расходы» и «Услуги ФФ» на уровне
        товара не считаются — это расходы всего бизнеса, а не конкретного товара, поэтому в разбивке там прочерк.
      </div>
    </div>
  );
}
