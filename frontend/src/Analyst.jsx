  ...
  return (
    <div>
      ...
      <div className="report-note">
        ...
        сайта — ничего дополнительно вводить не нужно. Маржа по товарам считается той же формулой, что и в
        «Отчёте» (себестоимость FIFO, комиссия, доставка, налог, маркетинг), просто по календарным месяцам,
        затронутым выбранным периодом. Каждое нажатие «Сформировать отчёт» расходует токены API — учитывайте
        это при частом использовании. Каждый сформированный отчёт сохраняется — «История отчётов» ниже
        позволяет открыть его позже без повторной генерации и удалить, если больше не нужен.
      </div>
    </div>
  );
}
