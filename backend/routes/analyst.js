...
const { aggregateKaspiPayMonthly, fetchOtherExpensesByMonth, MAIN_CITIES, SELF_BUY_CITIES } = require('./reports');
...
async function getMonthlyReportText() {
  const [monthsMainCities, otherExpensesByMonth] = await Promise.all([
  ...
  const rows = monthsMainCities.slice(0, 6).map((row) => {
    ...
    return { ...row, other_expenses: otherExpenses, net_profit: netProfit, margin, roi };
  });
  ...
  return rows
    .map((r) => (
      ...
      `комиссия ${fmt(r.commission)}, доставка ${fmt(r.delivery)}, налоги ${fmt(r.taxes)}, прочие расходы ${fmt(r.other_expenses)}, ` +
      `чистая прибыль ${fmt(r.net_profit)}, маржа ${pct(r.margin)}, ROI ${pct(r.roi)}`
    ))
    .join('\n');
}
