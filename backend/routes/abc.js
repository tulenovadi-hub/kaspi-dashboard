...
// Границы XYZ по коэффициенту вариации недельных продаж.
//
// НЕ классические 10%/25% — те придуманы для складов, отгружающих сотни одинаковых единиц
// в неделю, и на маркетплейсе бесполезны. Проверено на живых данных за июнь–август 2026:
// при порогах 10/25 ВСЕ товары до единого попали в Z, включая те, что продавались все 13 недель
// из 13 без единого пропуска. Реальные коэффициенты вариации там — от 38% до 122%.
// Взят розничный вариант порогов (25% и 50%): он разделяет "продаётся ровно" и "продаётся
// рывками" на тех объёмах, которые есть на самом деле.
const XYZ_X = 0.25;
const XYZ_Y = 0.5;
